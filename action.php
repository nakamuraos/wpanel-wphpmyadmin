        ...
        if ($dir == null || !is_dir(processDirectory($dir))) {
            ...
            if ($option === 0) {
                ...
                if (isset($_POST['submit']) && isset($_POST['is_action'])) {
                    ...
                    else if (!zips($dir, $entry, processDirectory($_POST['path'] . '/' . $_POST['name']), isset($_POST['is_delete'])))
                ...
                echo '<div class="list">
                    <form action="action.php?dir=' . $dirEncode . $pages['paramater_1'] . '" method="post">
                        ...
                        <input type="text" name="name" value="archive.zip" size="18"/><br/>
                        <span class="bull">&bull;</span>Đường dẫn lưu:<br/>
                        <input type="text" name="path" value="' . $dir . '" size="18"/><br/>
                        <input type="checkbox" name="is_delete" value="1"/> Xóa nguồn<br/>
                        <input type="hidden" name="is_action" value="1"/>
                        <input type="hidden" name="option" value="' . $option . '"/>';
                ...
                if (isset($_POST['submit']) && isset($_POST['is_action'])) {
                    ...
                    if (!$isFailed) {
                        ...
                        foreach ($entry AS $k => $e) {
                            ...
                            if (!@rename($entryPath . '-' . $rand, $dir . '/' . $modifier[$k])) {
                                $isSucceed = false;
