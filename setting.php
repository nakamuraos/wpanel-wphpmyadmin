<?php define('_PHONHO_HPA', true);
    ...
    if (IS_LOGIN) {
        $title = 'Cài đặt';
        $ref = isset($_POST['ref']) ? $_POST['ref'] : $_SERVER['HTTP_REFERER'];
        ...
        if (isset($_POST['submit'])) {
            ...
            if (empty($username)) {
            ...
            } else if (!empty($passwordO) && md5(md5($passwordO)) != $configs['password']) {
                ...
                if (createConfig($username, (!empty($passwordN) ? md5(md5($passwordN)) : $configs['password']), $pageList, $pageFileEdit, $pageFileEditLine, false)) {
                    include 'incfiles/PATH_CONFIG';
        ...
        <div class="tips"><img src="icon/tips.png"/> Mật khẩu để trống nếu không muốn thay đổi, phân trang danh sách, sửa văn bản thường, sửa văn bản theo dòng để trống nếu không muốn phân trang</div>
