    ...
    if (IS_LOGIN) {
        ...
        if ($dir == null || $name == null || !is_file(processDirectory($dir . '/' . $name))) {
            ...
            echo '<ul class="info">';
                ...
                echo '<li><strong>Tên</strong>: <span>' . $name . '</span></li>
                <li><strong>Kích thước</strong>: <span>' . size(filesize($path)) . '</span></li>
                <li><strong>Chmod</strong>: <span>' . getChmod($path) . '</span></li>';

                if ($isImage)
                    echo '<li><strong>Độ phân giải</strong>: <span>' . $pixel[0] . 'x' . $pixel[1] . '</span></li>';

                echo '<li><strong>Định dạng</strong>: <span>' . ($format == null ? 'Không rõ' : $format) . '</span></li>
                <li><strong>Ngày sửa</strong>: <span>' . @date('d.m.Y - H:i', filemtime($path)) . '</span></li>
            </ul>
            <div class="title">Chức năng</div>
            <ul class="list">';
